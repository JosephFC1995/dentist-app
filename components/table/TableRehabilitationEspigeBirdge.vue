  ...
  <div>
    ...
    <div class="table-general">
      ...
      >
        ...
        <FormRehabilitationEspigeBirdge @close="closeDrawer" :newData="newForm" :form="form" @reload="reload" />
      </a-drawer>
    </div>
  </div>
</template>
...
<script>
import FormRehabilitationEspigeBirdge from '~/components/form/FormRehabilitationEspigeBirdge'
import { mapState, mapMutations, mapGetters, mapActions } from 'vuex'

export default {
  components: {
    FormRehabilitationEspigeBirdge,
  },
  data() {
    return {
      detailDrawer: {},
  ...
  methods: {
    ...
    async deleteRow($event) {
      ...
      let response = false
      response = await this.$axios.$delete(`/rehabilitation_table_espige/${$event.id}`).catch((errors) => {
  ...
  watch: {
    rehabilitationRehabilitationTableEspigeSelect(newValue, oldValue) {
  ...
  mounted() {
    this.data =
      this.rehabilitationRehabilitationTableEspigeSelect.length > 0
        ? _.cloneDeep(this.rehabilitationRehabilitationTableEspigeSelect)
        : []
  },
}
</script>
